...
**任务清单：**
- [ ] 实现组件：`FileUpload.jsx`, `FactList.jsx`, `ConflictViewer.jsx`
- [ ] 对接后端 API
- [ ] 响应式布局适配
...
**任务清单：**
- [ ] 实现规则引擎
  - 规则1：如果某章节冲突密度高 → 推荐"重点检查该章节逻辑"
  - 规则2：如果缺少数据来源 → 推荐"补充引用文献"
  - 规则3：如果描述与参考图不一致 → 推荐"更新框架图或修正文字"
- [ ] 设计推荐 Prompt（基于 LLM）
  ```python
  RECOMMENDATION_PROMPT = """
  文档分析结果：
  - 总冲突数：{conflict_count}
  - 高风险章节：{high_risk_sections}
  - 未验证事实：{unverified_facts}
  
  请作为论文写作顾问，给出3-5条具体的改进建议。
  """
  ```
- [ ] 实现推荐 API
  ```python
  @app.get("/api/recommendations/{document_id}")
  async def get_recommendations(document_id: str)
  ```
...
- [ ] 实现段落续写/润色
  ...
  - 输出：建议的下一段内容
- [ ] 术语一致性检查
  - 检测全文是否统一使用术语（如"用户"vs"使用者"）

**注意：** 这些功能较复杂，建议在核心功能稳定后再开发

### 模块5.3：参考文献智能管理 [补充功能]
**功能：** 自动化引文格式调整与查漏补缺

**任务清单：**
- [ ] 识别文中引用标记（[1], (Author, Year)）
- [ ] 与参考文献列表比对，检测：
  - 有引用无文献
  - 有文献无引用
  - 格式不统一（GB/T 7714 vs APA）
- [ ] 实现一键格式化工具
...
### 1. 先跑通最小可行产品（MVP）
**目标：** 能上传文档 → 提取事实 → 检测基本冲突

### 2. Prompt 工程是关键
- 多轮测试优化 Prompt
- 使用 Few-shot Learning（给示例）
...
### 4. 分工建议（3人团队示例）
- **成员A（算法/Agent）：** 负责 Prompt 设计、LLM 调用、事实提取/冲突检测逻辑
- **成员B（架构/后端）：** 负责 FastAPI 接口、Redis 集成、Docker 配置
- **成员C（前端/可视化）：** 负责 React 界面、结果展示、Dashboard
...
### 硬伤防范
- [ ] 确保 `docker-compose up` 一键启动
- [ ] 提供测试数据（sample.docx）
- [ ] README 必须包含详细的运行步骤

### 严重违规防范
- [ ] 所有参考的开源代码必须在 README 注明
- [ ] 分工文档如实填写，避免造假
- [ ] 使用 Git 记录每人的 Commit 历史
...
# 2. 配置环境变量
cp .env.example .env
# 编辑 .env，填入 CLAUDE_API_KEY / OPENAI_API_KEY
